...
def evtab(Eval_DF_mine, prediction_columns, nhdreach, observation_column, mod):
    ...
    flowcols = [f"{mod}_flow", 'flow_cms', 'NWM_flow']
    ...
    EOY_obs_vol_af = SupplyEval["flow_cms"].iloc[-1]
    ...
    Supplystats = [Eval_DF_mine['station_id'].drop_duplicates()[0], nhdreach, Srmse[0], Srmse[1],  Spbias[0], Spbias[1], Skge[0], Skge[1], EOY_obs_vol_af, EOY_nwm_vol_af,EOY_mod_vol_af,NWM_vol_diff_af,Mod_vol_diff_af, NWM_Perc_diff, Mod_Perc_diff ] # Smape[0], Smape[1],

    return sitestats, Supplystats


def EvalTable(yhat_test, data_test, model_name):


    
    df_eval = data_test.copy()
    df_eval[f'{model_name}_flow'] = yhat_test
    prediction_columns = ['NWM_flow', f"{model_name}_flow"]
    observation_column = 'flow_cms'
    result_daily, result_cumulative = evtab(df_eval, prediction_columns, '10375648', observation_column, model_name)
    ...
    cols = ['USGSid', 'NHDPlusid', 'NWM_RMSE', f"{model_name}_RMSE", 'NWM_PBias', f"{model_name}_PBias", 
            'NWM_KGE', f"{model_name}_KGE"] #, 'NWM_MAPE',  f"{model_name}_MAPE"]
    ...
    supcols = ['USGSid', 'NHDPlusid', 'NWM_RMSE', f"{model_name}_RMSE", 'NWM_PBias', f"{model_name}_PBias", 
            'NWM_KGE', f"{model_name}_KGE", 'Obs_vol', 'NWM_vol', f"{model_name}_vol",
            'NWM_vol_err', f"{model_name}_vol_err", 'NWM_vol_Perc_diff', f"{model_name}_vol_Perc_diff"] # 'NWM_MAPE',  f"{model_name}_MAPE", 
